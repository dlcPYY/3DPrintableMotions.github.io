...
let dragresize_ev = new DragResize('dragresize',
{ minWidth: 280, minHeight: 130, minLeft: 20, minTop: 20, maxLeft: '80%', maxTop: '80%' });
dragresize_ev.isElement = function(elm) {
    if (elm.className && elm.className.indexOf('drsElement') > -1) return true;
};
dragresize_ev.isHandle = function(elm) {
    if (elm.className && elm.className.indexOf('drsMoveHandle') > -1) return true;
};
dragresize_ev.ondragfocus = function() { console.log('dragfocus')};
dragresize_ev.ondragstart = function(isResize) { console.log('isResize:' + isResize);console.log('ondragstart')};
dragresize_ev.ondragmove = function(isResize) { console.log('isResize:' + isResize);console.log('ondragmove')};
dragresize_ev.ondragend = function(isResize) { 
    space2dchart.resize()
};
dragresize_ev.ondragblur = function() {console.log('ondragblur') };

let node_ev = document.getElementById('drs-panel-space')
dragresize_ev.apply(node_ev)




let dragresize_proprity = new DragResize('dragresize',
{ minWidth: 10, minHeight: 10, minLeft: 0, minTop: 0, maxLeft: 1000, maxTop: 1000 });
dragresize_proprity.isElement = function(elm) {
    if (elm.className && elm.className.indexOf('drsElement') > -1) return true;
};
dragresize_proprity.isHandle = function(elm) {
    if (elm.className && elm.className.indexOf('drsMoveHandle') > -1) return true;
};
dragresize_proprity.ondragfocus = function() { console.log('dragfocus')};
dragresize_proprity.ondragstart = function(isResize) { console.log('isResize:' + isResize);console.log('ondragstart')};
dragresize_proprity.ondragmove = function(isResize) { console.log('isResize:' + isResize);console.log('ondragmove')};
dragresize_proprity.ondragend = function(isResize) { 
    space2dchart.resize()
};
dragresize_proprity.ondragblur = function() {console.log('ondragblur') };

let node_proprity = document.getElementById('drs-filter')
dragresize_proprity.apply(node_proprity)